...
class VideoPlayerProcessor {
  constructor(
    ...
    this.videoData.richText = customizedParamsJSON.richText;
    this.videoData.richButtonText = customizedParamsJSON.richButtonText;
  ...
  async initializePlayer(materialId, styleIdentifier) {
    ...
    if (!wrapper) {
      console.log("No wrapper found", styleIdentifier);
    }
      ...
      try {
        ...
        Promise.all([this.loadCss(), this.loadJs()])
          .then(() => {
            ...
            if (
              this.videoData.richShow === "t" &&
              (this.videoData.ctaLayout === "top-right" ||
                this.videoData.ctaLayout === "bottom-left")
            ) {
              this.videoData.ctaLayout = "below-video";
            }

            if (this.videoData.linkText) {
              ...
              document
                ...
                .addEventListener("click", () => this.shareOnFacebook(link));
              document
                .getElementById("twitter-btn")
                .addEventListener("click", () => this.shareOnTwitter(link));
              document
                .getElementById("linkedin-btn")
                .addEventListener("click", () => this.shareOnLinkedIn(link));
              document
                .getElementById("whatsapp-btn")
                .addEventListener("click", () => this.shareOnWhatsApp(link));
            } else {
              let overlayHTML = this.videoData.link
                ? '<a class="overlay" target="_blank" href="' +
                  this.videoData.link +
                  '"></a>'
            ...
            if (this.videoData.richText && this.videoData.richShow === "t") {
              let muteIcon = wrapper.querySelector(".mute-icon");
              muteIcon.style.left = "unset";
              muteIcon.style.right = "10px";

              let overlayHTML = this.getRichTextHTML();
              this.addOverlayToWrapper(
              ...
              wrapper
                ...
                .addEventListener("click", (event) => {
                  ...
                  wrapper.querySelector(".overlay .rich-block").style.display =
                    "none";
                });
            }
            ...
            let ctaContent = ctaHover.querySelector(".content");
            ...
            if (this.videoData.player) {
              this.videoData.player.addEventListener("play", (event) => {
                ...
                if (!isVisible && this.getCurrentVideoPlayerElement()) {
                  this.getCurrentVideoPlayerElement().pause();
                }
              ...
              window.addEventListener("resize", () => {
                if (ctaHover && ctaValue) {
                  ...
                  let numberOfRow = Math.floor(
                    (ctaHoverHeight / 2 - 42) / 16.5
                  );
              ...
              overlay.addEventListener("mouseleave", () => {
                ...
                if (this.videoData.ctaShowOnHover === "t" && ctaHover) {
                  ...
                  if (richBlock && this.videoData.richShow === "t") {
                    richBlock.style.display = "block";
                  }
                }
              });
  ...
  addReplayButtonListener() {
    this.videoData.player.addEventListener("play", () => {
      if (this.videoData.ctaHover && this.videoData.ctaShowOnHover === "t") {
        ...
        if (this.videoData.richBlock && this.videoData.richShow === "t") {
          this.videoData.richBlock.style.display = "block";
        }
      }
    });
  }
  ...
  getRichTextHTML() {
    ...
    let opacity = this.videoData.richBgOpacity;
    let bgColor = this.videoData.richBgColor;
    let textColor = this.videoData.richTextColor;
    let buttonTextColor = this.videoData.richButtonTextColor;

    let richBgColor =
      "background-color: " +
      this.hexToRGBA(bgColor, this.convertOpacityToDecimal(opacity)) +
      ";";
    let closeButton = '<button class="close-button">&times;</button>';

    if (!richButtonText) {
      return (
        '<div class="rich-block" style="' +
        richBgColor +
        " color: " +
        textColor +
        ';">' +
    ...
    return (
      ...
      this.videoData.richText +
      "</span>" +
      '<a style="text-align: center; color: ' +
      buttonTextColor +
      ';" href="' +
      this.videoData.link +
      '">' +
  ...
  getCtaHTML() {
    ...
    switch (layoutPosition) {
      ...
      case "below-video":
        ctaAreaOuter =
          ...
          '<a href="' +
          link +
          '" class="btn btn-default" target="_blank" type="button" style="background:' +
          ctaButtonColor +
          "; color: " +
          ctaButtonTextColor +
          ';">' +
      ...
      case "tagline-bar":
        ...
        ctaBlockHTML =
          '<div class="cta-slide" style="color: ' +
          textColor +
          ';">' +
          '<div class="logo">' +
          '<img src="' +
    ...
    let ctaAreaHover =
      ...
      "</div>" +
      '<div class="button">' +
      '<input class="btn btn-default" type="button" data-copy-code="" style="border: 1px ' +
      ctaButtonColor +
      " solid; background:" +
      ctaButtonColor +
      ";color: " +
    ...
    return (
      ...
      '<div class="control-area" style=""></div>' +
      ctaAreaOuter +
      (layoutPosition === "below-video" || layoutPosition === "tagline-bar"
        ? ""
        : '<div class="buttonWrapper">' +
          shareSocialHtml +
          '<div class="button">' +
  ...
  eventTrigger(eventType, params = {}) {
    ...
    switch (eventType) {
      ...
      case "complete":
        ...
        if (this.videoData.ctaHover && this.videoData.ctaShowOnHover === "t") {
          if (this.videoData.richBlock) {
            this.videoData.richBlock.style.display = "none";
          }

          this.videoData.ctaHover.style.display = "flex";
          if (this.videoData.ctaContent) {
            const ctaHoverHeight = this.videoData.ctaHover.offsetHeight;
            const numberOfRow = Math.floor((ctaHoverHeight / 2 - 42) / 16.5);
            this.videoData.ctaContent.style.webkitLineClamp = numberOfRow;
          }
        }
